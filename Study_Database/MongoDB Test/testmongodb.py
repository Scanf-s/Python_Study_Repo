...
def insert_data():
    ...
    books = [
        ...
        {"title": "123123", "author": "asdf", "year": 2011, "genre": "Fantasy"},
    ...
    movies = [
        ...
        {"title": "343434", "director": "Scanf-d", "year": 2008, "rating": 7.0},
    ]
    db.movies.insert_many(movies)

    # 사용자 행동 데이터 삽입
    user_actions = [
        {"user_id": 1, "action": "click", "timestamp": "2023-04-09T13:00:00Z"},
        {"user_id": 1, "action": "view", "timestamp": "2023-04-09T13:00:00Z"},
        {"user_id": 1, "action": "view", "timestamp": "2023-04-09T13:00:00Z"},
        {"user_id": 1, "action": "click", "timestamp": "2023-04-12T08:00:00Z"},
        {"user_id": 1, "action": "view", "timestamp": "2023-04-12T08:00:01Z"},
...
def question1(db, genre):
    ...
    # The projection parameter takes a document of the following form: { <field1>: <value>, <field2>: <value> ... }
...
def question2(db):
    # Group
    # https://www.mongodb.com/docs/manual/reference/operator/aggregation/group/#syntax

    # Average
    # https://www.mongodb.com/docs/manual/reference/operator/aggregation/avg/#behavior

    # Sort
    # https://www.mongodb.com/docs/manual/reference/operator/aggregation/sort/#syntax
    movies = db.movies
    avg_by_director = movies.aggregate([
        {
            # mongodb 데이터베이스 안에 저장된 속성명을 $변수명 을 통해 가져올 수 있다.
...
def question3(db, user_id):
    ...
    # https://www.mongodb.com/docs/manual/reference/method/cursor.limit/#behavior
    recent_behavior = user_actions.find(
        {"user_id": user_id}
...
def question4(db):
    ...
    results = books_collection.aggregate([
        {
            "$group": {"_id": "$year", "count": {"$sum": 1}} # 년도별로 출판된 책 count
        },
        {
            "$sort": {"count": -1}
        }
...
def question5(db, user_id, date):
    ...
    user_actions.update_many(
        # 1번 중괄호에 해당하는 모든 사용자 행동 데이터를 가져와서
        {
            ...
            # less than
            # https://www.mongodb.com/docs/manual/reference/operator/query/lt/#examples
...
# 실제로 프로그램 debug 도구를 어떻게 사용하는지,
# debug program을 어떻게 활용해야하는지 궁금합니다.